---
title: Webpage Theme 
date: 2025-02-08 21:16:00 +0100 
categories: [2025 Trial-Run, Preparations] 
tags: [trial-run, training, personal, pictures, site]
image: 
 path: /assets/pics/20210806_TheLowlands.jpg
 alt: "<a href='/photos/#2021'><i class='fas fa-camera'></i> The Lowlands <i class='fas fa-up-right-from-square'></i> </a>"
description: "183 Days to Prepare. Choosing a Web Page Theming..." 
---


> **183** Days to Prepare. <br class='brS'/><i class='fas fa-location-dot'></i> *North-Holland, The Netherlands*. <br class='brSM'/>
> [<i class='fas fa-calendar-days'></i> Quick Overview ](/welcome/#quick-overview) <br/> 
{: .prompt-quick }


While I still need to restart with my complete training regimen, 
I do take long hikes almost every day. 
Also I've been to two singing lessons so far and I'm really enjoying it!

I do still need to make an appointment with my GP, 
because I can feel my feet and knees hurting after only short walks.


## Technical Update

I've made the decision to work with a new Jekyll Theme template 
called [Chirpy by Cotes Chung <i class='fas fa-up-right-from-square'></i>](https://chirpy.cotes.page/) <br/>
I’m getting the IDE for development to work and then will try to migrate from the old theme to the new. 
This theme has a lot more features, especially for a blog-like website.
The only downside so far seems to be that deploying the site is a bit more difficult.


But I'm hoping to be familiar enough with Jekyll by the time I'm traveling, 
so that I don't need to have a full IDE-suite to post from.


<br/>
**Southward Bound!** <br/>
{: .quote }
*Cheers,* <br/>
I-Jun <i class='fas fa-person-hiking fa-fw'></i><i class='fas fa-mountain-sun fa-fw'></i> <br/>
[<i class='fab fa-github fa-fw'></i>  Github.com @ MyPilgrimage](https://github.com/MyPilgrimage/MyPilgrimage.github.io) <br/>
[<i class='fab fa-reddit fa-fw'></i>  Reddit.com @ /u/I-Jun_Pilgrimage](https://www.reddit.com/user/I-Jun_Pilgrimage/) <br/>
[<i class='fas fa-envelope fa-fw'></i>  Mail @ iJun.Pilgrimage@gmail.com](mailto:iJun.Pilgrimage@gmail.com) <br/>

