---
title: Continuing 
date: 2025-02-06 22:00:00 +0100 
categories: [2025 Trial-Run, Preparations] 
tags: [trial-run, training, personal, site]
description: "185 Days to Prepare. Continuing with training and updating this page..." 
---


> **185** Days to Prepare. <br class='brS'/><i class='fas fa-location-dot'></i> *North-Holland, The Netherlands*. <br class='brSM'/>
> [<i class='fas fa-chart-line'></i> Quick Overview ](/welcome/#quick-overview) <br> 
{: .prompt-quick }


I try to keep persistent with my hiking-training, 
but I have to admit that my cardio training has somewhat fallen to the wayside. 
Slowly I'm trying to find my motivation again, 
and to keep on pushing myself.


Luckily working on this page seems to work!
{: .quote }


## Technical Update

I'm diving into the deployment method of Github Pages, named Jekyll. 
The idea is to create a reactive page where I can easily add blog-posts. 
I can see that it is possible with Jekyll, 
and since I have a little experience with React, 
I recognised a few things.


So this is a new programming-language for me, 
but we'll see if I can make it work. 
At the very least having my Blog Posts as separate files, 
makes navigating the files a lot easier to fix spelling and mistakes.


<br>
**Southward Bound!**
{: .quote }
*Cheers,* <br>
I-Jun <i class='fas fa-person-hiking fa-fw'></i><i class='fas fa-mountain-sun fa-fw'></i> <br>
[<i class='fab fa-github fa-fw'></i>  Github.com @ MyPilgrimage](https://github.com/MyPilgrimage/MyPilgrimage.github.io) <br>
[<i class='fab fa-reddit fa-fw'></i>  Reddit.com @ /u/I-Jun_Pilgrimage](https://www.reddit.com/user/I-Jun_Pilgrimage/) <br>
[<i class='fas fa-envelope fa-fw'></i>  Mail @ iJun.Pilgrimage@gmail.com](mailto:iJun.Pilgrimage@gmail.com) <br>
