---
title: Live but Weird
date: 2025-02-15 23:00:00 +0100 
categories: [Site, Update] 
tags: [personal, site]
image: 
 path: /assets/img/img_brokenWebsite.png
 alt: Broken website 
description: "176 Days to Prepare. The new site looks.. Weird..." 
---


> **176** Days to Prepare. <br class='brS'/><i class='fas fa-location-dot'></i> *North-Holland, The Netherlands*. <br class='brSM'/>
> [<i class='fas fa-calendar-days'></i> Quick Overview <i class='fas fa-up-right-from-square'></i>](/welcome/#quick-overview) 
{: .prompt-quick }


## The page is live!

But sadly it would seem I made a few mistakes in deploying the site. 


## Technical Update

![Broken website](/assets/img/img_brokenWebsite.png)
_Broken website. This doesn't look right..._
{: .img .w-50 .right }


I think it has to do with the fact that I used the original `_sass` folder for the style, 
while I should've used the assets/CSS instead.


Many elements on the site do not display correctly 
and there is also the issue with the Tags and Categories tabs. 
I wanted only to have one page for the Tags and Categories, 
but this created a deployment error. 
So this I will need to fix somehow, 
because I would still like the functionality of these Tags and Categories.


<br/>
**But anyway, the new site is live! Now only to fix it 😉**
{: .quote }
*Cheers,* <br/>
I-Jun <i class='fas fa-person-hiking fa-fw'></i><i class='fas fa-mountain-sun fa-fw'></i> <br/>
[<i class='fab fa-github fa-fw'></i>  Github.com @ MyPilgrimage](https://github.com/MyPilgrimage/MyPilgrimage.github.io) <br/>
[<i class='fab fa-reddit fa-fw'></i>  Reddit.com @ /u/I-Jun_Pilgrimage](https://www.reddit.com/user/I-Jun_Pilgrimage/) <br/>
[<i class='fas fa-envelope fa-fw'></i>  Mail @ iJun.Pilgrimage@gmail.com](mailto:iJun.Pilgrimage@gmail.com) <br/>